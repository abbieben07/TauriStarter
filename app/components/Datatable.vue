<template>
	<div @click="handleTableClick">
		...
		<table ref="view" class="table border-2 border-left-0 border-right-0 border-top border-bottom w-100">
...
class Datatable extends Vue {
	...
	mounted() {
		...
		$(() => {
			...
			$('.dt-search input').on('keydown', (e) => {
				if (e.key === 'Enter') {
					const search = $('.dt-search input').val() as string

					if (search.length > 0) {
						this.datatable.search(search).draw()(e.currentTarget as HTMLInputElement).value = ''
					}
				}
			})
		})
