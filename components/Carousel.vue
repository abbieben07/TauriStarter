<template>
	<div ref="view" class="carousel">
		<div v-show="indicators" class="carousel-indicators">
			<button v-for="i in slides" :key="i" type="button" :class="i - 1 === index ? 'active' : ''" @click="goTo(i)" />
		</div>
...
class Carousel extends Vue {
	...
	mounted() {
		...
		// @ts-ignore
		this.view.addEventListener('slid.bs.carousel', ({ to }) => (this.index = to))
	}
